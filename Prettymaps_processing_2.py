"""
QGIS Processing Script: Style OSM Map Layers (Prettymaps 2)
...
class StyleOSMMapAlgorithm(QgsProcessingAlgorithm):
    ...
    def style_polygons(self, layer, colors, feedback):
        ...
        beach_filter = ' OR '.join(beach_conditions)
        beach_symbol = QgsSymbol.defaultSymbol(2)
        beach_symbol.setColor(QColor(colors['beach']))
        beach_symbol.symbolLayer(0).setStrokeColor(QColor(colors['edge']))
        beach_symbol.symbolLayer(0).setStrokeWidth(0.3)
        beach_symbol.symbolLayer(0).setStrokeStyle(Qt.SolidLine)
        root_rule.appendChild(QgsRuleBasedRenderer.Rule(beach_symbol, 0, 0, beach_filter, 'Beaches'))
        ...
        farmland_filter = ' OR '.join(farmland_conditions)
        farmland_symbol = QgsSymbol.defaultSymbol(2)
        farmland_symbol.setColor(QColor(colors['farmland']))
        farmland_symbol.symbolLayer(0).setStrokeColor(QColor(colors['edge']))
        farmland_symbol.symbolLayer(0).setStrokeWidth(0.3)
        farmland_symbol.symbolLayer(0).setStrokeStyle(Qt.SolidLine)
        root_rule.appendChild(QgsRuleBasedRenderer.Rule(farmland_symbol, 0, 0, farmland_filter, 'Farmland'))
    ...
    def shortHelpString(self):
        ...
        """
        return """
        Prettymaps 2 - Apply prettymaps-inspired styling to OSM layers from ANY location.
        
        This algorithm styles:
        - Polygon layer: Buildings (3-color palette), parks, water, forests, 
          parking, plazas, beaches, farmland
        ...
        Colors:
        - Background: #F2F4CB (cream)
        - Green spaces: #D0F1BF
        - Forest: #64B96A
        - Water: #a1e3ff
        - Beaches: #fdeab7
        - Farmland: #ffaf00
        - Buildings: #FFC857, #E9724C, #C5283D (3-color palette)
        - Streets: #2F3737
        """
